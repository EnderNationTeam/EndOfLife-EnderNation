...
public class DefaultScoreboard extends ScoreboardBuilder {
    ...
    public void createScoreboard() {
        ...
        switch (EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId())) {
            case "Arbeitslos" -> setScore("  §8» §8§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
            case "Farmer" -> setScore("  §8» §a§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
            case "Holzfäller" -> setScore("  §8» §2§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
            case "Schlachter" -> setScore("  §8» §d§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
            case "Miner" -> setScore("  §8» §6§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
            case "Fischer" -> setScore("  §8» §9§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
        }
        setScore("§a§8§m                               ", 1);
        update();
    }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                switch (EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId())) {
                    ...
                    case "Holzfäller" -> setScore("  §8» §2§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
                    case "Schlachter" -> setScore("  §8» §d§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
                    case "Minenarbeiter" -> setScore("  §8» §6§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
                    case "Fischer" -> setScore("  §8» §9§l" + EndoflifeCore.getInstance().getJobAPI().getJob(player.getUniqueId()), 2);
                }
                setScore("§a§8§m                               ", 1);
            }
        }.runTaskTimer(EndoflifeCore.getInstance(), 0, 1);
    }
}
