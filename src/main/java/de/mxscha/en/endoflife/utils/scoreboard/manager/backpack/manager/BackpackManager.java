package de.mxscha.en.endoflife.utils.scoreboard.manager.backpack.manager;
    ...
    public void initPlayer(Player player) {
        if (!isUserExists(player.getUniqueId())) {
            if (isNameAlreadyInDataBase(player.getName())) {
                mySQL.update("update backpacks SET uuid=? WHERE name=?", player.getUniqueId().toString(), player.getName());
            } else
