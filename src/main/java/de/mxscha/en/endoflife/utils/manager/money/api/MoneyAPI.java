...
public class MoneyAPI implements IMoneyAPI {
    ...
    public void initPlayer(Player player) {
        if (!(isUserExists(player.getUniqueId()))) {
            if (!isNameAlreadyInDataBase(player.getName())) {
                mySQL.update("INSERT INTO money (uuid, name, money) VALUES (?,?,?)", player.getUniqueId().toString(), player.getName(), 0.00);
            } else {
    ...
    public void createTables() {
        ...
        mySQL.update("CREATE TABLE IF NOT EXISTS money (uuid VARCHAR(36) PRIMARY KEY, name VARCHAR(36), money DOUBLE(9,2))");
    }
}