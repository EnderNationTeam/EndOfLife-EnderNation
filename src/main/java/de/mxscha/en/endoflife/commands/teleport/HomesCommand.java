...
public class HomesCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(sender instanceof Player player) {
            ...
            textComponent.append(E).append(N).append(D).append(O).append(F).append(L).append(I).append(f).append(e).append(middle).append(Component.text("§7Homes: "));
            for(Home home : EndoflifeCore.getInstance().getHomeManager().getHomes(player).getList()) {
